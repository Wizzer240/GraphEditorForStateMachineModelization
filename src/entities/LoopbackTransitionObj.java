...
public class LoopbackTransitionObj extends TransitionObj implements Cloneable {
  ...
  private void setEndPts(int x, int y) {
    ...
    endCtrlPt = new Point();
    startCtrlPt.setLocation((int) (dist * Math.cos(angleStart))
        + startPt.getX(), dist * Math.sin(angleStart) + startPt.getY());
    endCtrlPt.setLocation((int) (dist * Math.cos(angleEnd)) + endPt.getX(),
        dist * Math.sin(angleEnd) + endPt.getY());
  ...
  public void paintComponent(Graphics g) {
    if (ready && currPage == myPage) {
      ...
      // g2D.drawLine((int)endPt.getX(),(int)endPt.getY(),(int)endPt.getX() +
      // (int)(arrowLength*Math.cos(alpha + adj)),(int)endPt.getY() -
       ...
       * 
       * double adj = Math.PI/6;
       * g2D.drawLine(eX,eY,eX + (int)(20*Math.cos(theta + adj)),eY -
       * (int)(20*Math.sin(theta + adj)));
       * g2D.drawLine(eX,eY,eX + (int)(20*Math.cos(theta - adj)),eY -
       * (int)(20*Math.sin(theta - adj)));
       */
      ...
      if (selectStatus != SelectOptions.NONE) {
        ...
        g2D.fillRect((int) endCtrlPt.getX() - 3, (int) endCtrlPt.getY() - 3, 7,
            7);
    ...
    if (currPage == myPage) {
      ...
      if (selectStatus == SelectOptions.BR) {
        ...
        if (step != 0) {
          ...
          startPt = stateBorderPts.get(startStateIndex);
          endPt = stateBorderPts.get(endStateIndex);
          startCtrlPt.setLocation((int) (lengthS
              * Math.cos(ctrlAngleS - step * (2 * Math.PI / 36)) + startPt
              .getX()), (int) (-lengthS
              * Math.sin(ctrlAngleS - step * (2 * Math.PI / 36)) + startPt
              .getY()));
          endCtrlPt
              .setLocation((int) (lengthE
                  * Math.cos(ctrlAngleE - step * (2 * Math.PI / 36)) + endPt
                  .getX()), (int) (-lengthE
                  * Math.sin(ctrlAngleE - step * (2 * Math.PI / 36)) + endPt
                  .getY()));
        }
      }

      loop.setCurve(startPt.getX(), startPt.getY(), startCtrlPt.getX(),
  ...
  public boolean setSelectStatus(int x, int y) {
    ...
    if (currPage == myPage) {
      if (selectStatus != SelectOptions.BR || !loop.contains(x, y)) {
        ...
        // ctrlAngleS =
        // getAngle(startCtrlPt,startPt)-(36-startStateIndex)*2*Math.PI/36;
        // ctrlAngleE =
        // getAngle(endCtrlPt,endPt)-(36-endStateIndex)*2*Math.PI/36;
  ...
  public void updateObj() {
    ...
    if (state.getSelectStatus() != SelectOptions.TXT) {
      ...
      startCtrlPt.setLocation((int) (lenS * Math.cos(angleS)) + startPt.getX(),
          -lenS * Math.sin(angleS) + startPt.getY());
      endCtrlPt.setLocation((int) (lenE * Math.cos(angleE)) + endPt.getX(),
          -lenE * Math.sin(angleE) + endPt.getY());
  ...
  public Point getCenter(int page) {
    ...
    return new Point((int) (len * Math.cos(index * 2 * Math.PI / 36) + bord
        .getX()),
        (int) (len * Math.sin(index * 2 * Math.PI / 36) + bord.getY()));
